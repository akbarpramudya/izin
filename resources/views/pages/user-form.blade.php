<x-modal title="Form User" action="{{ $action }}">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <x-forms.input type="password" label="Password Konfirmasi" name="password_confirmation" />
        </div>
        <div class="col-md-12">
        ...
        <div class="col-md-6">
            <x-forms.select name="status_karyawan" label="Status Karyawan">
                @foreach (['Tetap' => 'tetap', 'Kontrak' => 'kontrak'] as $key => $value)
            ...
            <table class="table">
                ...
                <tbody id="listAtasan">
                    @foreach ($data->atasan as $item)
                        <tr>
                            <td><button type="button" class="btn btn-sm btn-danger btn-delete me-1"><i class="ti ti-trash"></i></button>{{ $item->nama }}</td>
                            <td>{{ $item->email }}</td>
